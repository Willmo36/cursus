...
If the workflow blocks on a signal (e.g. `waitFor`), `runWorkflow` returns immediately with `state: "waiting"`. The snapshot captures events up to the blocking point.
