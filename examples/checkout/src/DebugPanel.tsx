// ABOUTME: Debug panel that shows live workflow event logs and provides storage controls.
// ABOUTME: Reads events from localStorage and auto-refreshes on storage changes.
import { useCallback, useEffect, useState } from "react";
import type { WorkflowEvent } from "react-workflow";
import { storage } from "./storage";

const WORKFLOW_IDS = ["profile", "checkout"];
		...
		>
			...
			>
				...
				{!open && (
					<span style={{ color: "#888", marginLeft: 8 }}>
						({logs.reduce((n, l) => n + l.events.length, 0) || "..."} events)
					</span>
				)}
			</button>
			...
			{open && (
				<div style={{ maxHeight: 300, overflow: "auto", padding: "8px 12px" }}>
					...
					{logs.map((log) => (
						<WorkflowLog key={log.id} id={log.id} events={log.events} />
					))}
				</div>
			)}
		</div>
...
function WorkflowLog({ id, events }: { id: string; events: WorkflowEvent[] }) {
	return (
