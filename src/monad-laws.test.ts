...
import { activity, all, child, handler, loop, loopBreak, publish, receive, race, sleep, workflow } from "./types";
import type { Publishes, Receives, Requirements, WorkflowGenerator, WorkflowReturn } from "./types";
...
describe("Monad laws", () => {
	...
	describe("Requirement inference", () => {
		...
		it("query propagates Receives requirement", () => {
			const w = workflow(function* () {
				const user = yield* receive("login").as<{ name: string }>();
				return user.name;
			});
			type R = Requirements<typeof w>;
			const _check: AssertEqual<R, Receives<"login", { name: string }>> = true;
			void _check;
			void w;
		});
		...
		it("child propagates inner workflow requirements", () => {
			const childWf = workflow(function* () {
				const name = yield* receive("name").as<string>();
			...
			type R = Requirements<typeof w>;
			const _check: AssertEqual<R, Receives<"name", string>> = true;
			void _check;
			void w;
		});
		...
		it("race propagates union of branch requirements", () => {
			const w = workflow(function* () {
				return yield* race(
					...
					receive("payment").as<number>(),
				);
			});
			type R = Requirements<typeof w>;
			const _check: AssertEqual<R, Receives<"login", { user: string }> | Receives<"payment", number>> = true;
			void _check;
			void w;
		});
		...
		it("all propagates union of branch requirements", () => {
			const w = workflow(function* () {
				return yield* all(
					receive("login").as<{ user: string }>(),
					receive("config").as<{ url: string }>(),
				);
			});
			type R = Requirements<typeof w>;
			const _check: AssertEqual<R, Receives<"login", { user: string }> | Receives<"config", { url: string }>> = true;
			void _check;
			void w;
		});
		...
		it("handler with single .on() propagates Signal requirement with inferred payload", () => {
			...
			type R = Requirements<typeof w>;
			const _check: AssertEqual<R, Receives<"input", string>> = true;
			void _check;
			void w;
		});
		...
		it("handler propagates specific Signal requirements inferred from handler payload", () => {
			...
			type R = Requirements<typeof w>;
			const _check: AssertEqual<R, Receives<"greet", string>> = true;
			void _check;
			void w;
		});
		...
		it("handler with multiple .on() calls propagates union of typed Signal requirements", () => {
			...
			type R = Requirements<typeof w>;
			const _check: AssertEqual<R, Receives<"greet", string> | Receives<"farewell", number>> = true;
			void _check;
			void w;
		});

		it("query requirements do not widen to Receives<string, any>", () => {
			...
			const _notWide1: AssertNotEqual<R1, Receives<string, any>> = true;
			const _exact1: AssertEqual<R1, Receives<"submit", { data: number }>> = true;
			...
			const _notWide2: AssertNotEqual<R2, Receives<string, any>> = true;
			const _exact2: AssertEqual<R2, Receives<"greet", string> | Receives<"farewell", number>> = true;
		...
		it("handler propagates Publishes from handler bodies", () => {
			...
			type R = Requirements<typeof w>;
			const _check: AssertEqual<R, Receives<"go", undefined> | Publishes<number>> = true;
			void _check;
			void w;
		});

		it("handler propagates Receives requirement from handler body", () => {
			const w = workflow(function* () {
				return yield* handler()
					.on("go", function* (_payload: undefined, done) {
						const config = yield* receive("config").as<{ url: string }>();
						yield* done(config.url);
					})
					.as<string>();
			});
			type R = Requirements<typeof w>;
			const _hasSignal: AssertEqual<Receives<"go", undefined> extends R ? true : false, true> = true;
			const _hasOutput: AssertEqual<Receives<"config", { url: string }> extends R ? true : false, true> = true;
		...
		it("nested handler propagates inner signals up through outer handler", () => {
			...
			type R = Requirements<typeof w>;
			const _hasOuter: AssertEqual<Receives<"start", undefined> extends R ? true : false, true> = true;
			const _hasInner: AssertEqual<Receives<"confirm", string> extends R ? true : false, true> = true;
			void _hasOuter; void _hasInner; void w;
		});

		it("receive().as<V>() infers workflow id and types value", () => {
			const w = workflow(function* () {
				const config = yield* receive("config").as<{ url: string }>();
				return config.url;
			});
			type R = Requirements<typeof w>;
			const _check: AssertEqual<R, Receives<"config", { url: string }>> = true;
			void _check;
			void w;
		});

		it("receive() without .as() infers unknown value", () => {
			const w = workflow(function* () {
				const data = yield* receive("payment");
				return data;
			});
			type R = Requirements<typeof w>;
			const _check: AssertEqual<R, Receives<"payment", unknown>> = true;
			void _check;
			void w;
		});
		...
		it("query requirements do not widen to Receives<string, any>", () => {
			const w = workflow(function* () {
				return yield* receive("config").as<{ url: string }>();
			});
			type R = Requirements<typeof w>;
			const _notWide: AssertNotEqual<R, Receives<string, any>> = true;
			const _exact: AssertEqual<R, Receives<"config", { url: string }>> = true;
			void _notWide; void _exact; void w;
		});

		it("query in handler body propagates Receives requirement up", () => {
			const w = workflow(function* () {
				return yield* handler()
					.on("go", function* (_payload: undefined, done) {
						const config = yield* receive("config").as<{ url: string }>();
		...
		it("multiple operations accumulate requirements as union", () => {
			...
			const _check: AssertEqual<
				R,
				| Receives<"config", { url: string }>
				| Receives<"login", { name: string }>
				| Publishes<number>
			> = true;
			void _check;
			void w;
		...
		it("receive().as<V>() infers query label and types value", () => {
			const w = workflow(function* () {
				const config = yield* receive("config").as<{ url: string }>();
				return config.url;
			});
		...
		it("query in all propagates Receives requirements", () => {
			const w = workflow(function* () {
				return yield* all(
					receive("payment").as<{ amount: number }>(),
					receive("profile").as<{ name: string }>(),
				);
			});
			type R = Requirements<typeof w>;
			const _check: AssertEqual<R, Receives<"payment", { amount: number }> | Receives<"profile", { name: string }>> = true;
			void _check;
			void w;
		});

		it("query in race propagates Receives requirements", () => {
			const w = workflow(function* () {
				return yield* race(
					receive("fast").as<string>(),
					receive("slow").as<number>(),
				);
			});
			type R = Requirements<typeof w>;
			const _check: AssertEqual<R, Receives<"fast", string> | Receives<"slow", number>> = true;
			void _check;
			void w;
		});
		...
		it("loop propagates requirements from body", () => {
			...
			type R = Requirements<typeof w>;
			const _check: AssertEqual<R, Receives<"input", string> | Publishes<string>> = true;
			void _check;
			void w;
		});
	...
	describe("Free functions", () => {
		...
		it("query free function carries Receives requirement", async () => {
			const w = workflow(function* () {
			...
			type R = Requirements<typeof w>;
			const _check: AssertEqual<R, Receives<"login", string>> = true;
			void _check;
		...
		it("receive().as<V>() infers query name and types payload", async () => {
			const w = workflow(function* () {
			...
			type R = Requirements<typeof w>;
			const _check: AssertEqual<R, Receives<"pay", { amount: number }>> = true;
			void _check;
		...
		it("receive() without .as() infers unknown payload", async () => {
			...
			type R = Requirements<typeof w>;
			const _check: AssertEqual<R, Receives<"submit", unknown>> = true;
			void _check;
		...
		it("free functions compose with precise yield types", async () => {
			...
			// Only Receives<"greet", string> — no Result, Published, or Publishes
			type R = Requirements<typeof w>;
			const _check: AssertEqual<R, Receives<"greet", string>> = true;
			void _check;
