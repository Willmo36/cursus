...
describe("Interpreter", () => {
	...
	describe("snapshot", () => {
		it("update sets snapshot value readable via getter", async () => {
			const workflow: WorkflowFunction<
				string,
				Record<string, unknown>,
				Record<string, never>,
			...
			expect(interpreter.snapshot).toBe(42);
		});

		it("update notifies change listeners", async () => {
			const workflow: WorkflowFunction<
				string,
				Record<string, unknown>,
				Record<string, never>,
			...
			const interpreter = new Interpreter(workflow, new EventLog());
			const calls: unknown[] = [];
			interpreter.onStateChange(() => calls.push(interpreter.snapshot));
		...
		it("snapshot is undefined when update is never called", async () => {
			const workflow: WorkflowFunction<string> = function* (ctx) {
				return yield* ctx.activity("greet", async () => "hello");
			};
			...
			expect(interpreter.snapshot).toBeUndefined();
		});

		it("snapshot is populated during replay", async () => {
			const workflow: WorkflowFunction<
				string,
				Record<string, unknown>,
				Record<string, never>,
			...
			expect(interpreter.snapshot).toBe(99);
		});
	});
